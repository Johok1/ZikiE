...
import zinxs.wiki.account.AccountService;
...
public class ProfileService {
    ...
    public String setBannerImage(String tempToken, MultipartFile image){
        try {
            Account account = getAccount(tempToken);
            account.setBannerImage(image.getBytes());
            accountRepository.save(account);
            return "true";
        }catch (Exception e) {
            throw new RuntimeException(e);
    ...
    public byte[] getBannerImage(String tempToken){
        Account account = getAccount(tempToken);
        return account.getBannerImage();
    }

    public String checkProfilePin(String tempToken, String pin){
        Account account = getAccount(tempToken);
        String acntPin = account.getPincode();
        if(acntPin.equals(pin)){
            return "true";
        }else {
            return "false";
        }
    ...
    public String getProfileBio(String tempToken){
        Account account = getAccount(tempToken);
        return account.getBio();
    }

    public String setProfileBio(String tempToken, String bio){
        Account account = getAccount(tempToken);
        account.setBio(bio);
        accountRepository.save(account);
        return account.getBio();
