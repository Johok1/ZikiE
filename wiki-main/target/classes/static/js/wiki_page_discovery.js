...
class PageCard {
	...
	handlePageRedirect = (handler) => {
		let id = this.name
		this.hrefDiv.addEventListener("click", function () {
			handler(id)
		})
	}
}
...
class Model {
	constructor(view, controller) {
		...
		this.controller.getWikiPages(this.token, this.wikiId)
			.then(response => response.text())
			.then(response => {
				console.log("getWikiPages response: " + response) 
	...
	redirectPageHandler = (id) => {
		console.log("redirectHandler id: " + id)
		setCookie("wikiPageId",id,1)
	}

	makePages(list) {
		this.view.pagesDiv.innerHTML = ""
		for (let x = 0; x < list.length; x++) {
			if (list[x] != undefined && list[x] != "") {
				let page = new PageCard(this.view.pagesDiv, list[x]);
				page.handlePageRedirect(this.redirectPageHandler)
			}
		}
	}
	...
	searchInputHandler = () => {
		...
		for (let x = 0; x < userList.length; x++) {
			...
			if (sub.includes(searchsub)) {
				if (userList[x] != undefined && userList[x] != "") {
