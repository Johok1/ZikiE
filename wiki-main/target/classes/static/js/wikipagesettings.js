...
class Model {
	constructor(view, controller) {
		...
		this.controller.getWikiTags(this.token, this.wikiId)
			.then(response => response.text())
			.then(response => {
				this.wikiTags = response 
			

			this.controller.getWikiPageTags(this.token, this.wikiId, this.wikiPageId)
				.then(response => response.text())
				.then(response => {
					console.log("wikitags " + this.wikiTags)
					console.log("wikipagetags " + this.users)
					...
					this.loadTags(this.wikiTags.split(",").flat(), this.addWikiPageTag, this.removeWikiPageTag)
	...
	removeWikiPageTag = (tag) => {
		this.controller.removeWikiPageTag(this.token, this.wikiId, this.wikiPageId, tag.tagname)
			.then(response => response.text())
			.then(response => {
				if (response == "true") {
					tag.enabled = false
	...
	addWikiPageTag = (tag) => {
		this.controller.addWikiPageTag(this.token, this.wikiId, this.wikiPageId, tag.tagname)
			.then(response => response.text())
			.then(response => {
				if (response == "true") {
					tag.enabled = true
	...
	loadTags(taglist,addPageFunction,removePageFunction) {
		for (let x = 0; x < taglist.length; x++) {
			if (taglist[x] != "") {
				...
				if (this.view.internaltagslist.includes(tag.tagname)) {
					tag.setEnabled()
				}
	...
	handleCreateInternalTagInput = () => {
		...
		this.controller.addWikiPageTag(this.token, this.wikiId, this.wikiPageId, value)
			.then(response => response.text())
			.then(response => {
				if (response == "true") {
...
class Controller {
	...
	getWikiPageTags(token, wikiId,wikiPageId) {
		return fetch(this.fetch_url_wiki + "/getWikiPageTags/" + token + "/" + wikiId + "/" + wikiPageId, {
			method: 'GET',
			headers: {
				'Access-Control-Allow-Origin': '*',
				'Access-Control-Allow-Origin': '*',
	...
	addWikiPageTag(token, wikiId, wikiPageId, value) {
		return fetch(this.fetch_url_wiki + "/addWikiPageTag/" + token + "/" + wikiId + "/" + wikiPageId + "/" + value, {
			method: 'POST',
			headers: {
				'Access-Control-Allow-Origin': '*',
				'Access-Control-Allow-Origin': '*',
	...
	removeWikiPageTag(token, wikiId, wikiPageId, value) {
		return fetch(this.fetch_url_wiki + "/removeWikiPageTag/" + token + "/" + wikiId + "/" + wikiPageId + "/" + value, {
			method: 'POST',
			headers: {
				'Access-Control-Allow-Origin': '*',
				'Access-Control-Allow-Origin': '*',
